...
export default async function LoginPage({
  ...
  if (session?.user && reason !== 'denied' && !params.error) redirect('/app');
  ...
  if (authUnavailable) message = 'Сервіс входу тимчасово недоступний. Спробуйте ще раз за кілька хвилин.';
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-8">
      ...
      >
        ...
        <section className="p-8 sm:p-12"><div className="mb-10 flex items-center gap-3 lg:hidden"><div className="rc-gradient-brand flex h-11 w-11 items-center justify-center rounded-xl font-bold text-white">R</div><p className="font-serif text-2xl">{t('productName')}</p></div><p className="rc-kicker">Secure workspace</p><p className="mt-3 font-serif text-4xl text-text-primary">{t('productName')}</p><p className="mt-2 text-sm text-text-secondary">{t('tagline')}</p><p className="mt-8 text-sm leading-6 text-text-secondary">{t('staffOnly')}</p><p className="mt-4 text-sm leading-6 text-text-secondary">{message}</p>{reason === 'denied' || params.error || authUnavailable ? <p role="alert" className="mt-4 rounded-lg bg-danger/10 p-3 text-sm text-danger">{message}</p> : null}<div className="mt-8"><LoginForm returnTo={safeReturnTo(params.returnTo)} /></div><p className="mt-6 text-xs text-text-secondary">{t('notDiagnostic')}</p></section>
        {session?.user && (reason === 'denied' || params.error) ? (
          <form action={logoutStaff} className="mt-4">
            <button type="submit" className="text-sm text-danger underline">Вийти з Neon Auth</button>
          </form>
        ) : null}
      </main>
    </div>
