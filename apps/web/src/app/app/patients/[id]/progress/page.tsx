...
export default async function PatientProgressPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ period?: string; from?: string; to?: string; timelineCategory?: string; timelineCursor?: string }> }) {
  ...
  if (!canReadProgress(me) || !canReadClinicalTimeline(me)) return <div className="rounded-md border border-danger/30 bg-danger/5 p-5"><h1 className="font-serif text-2xl">Динаміка недоступна</h1><p className="mt-2 text-sm">Недостатньо клінічних прав для перегляду.</p></div>;
  ...
  try {
    ...
    return <div className="space-y-4"><h1 className="font-serif text-3xl">Динаміка пацієнта</h1><div role="alert" className="rounded-md border border-danger/30 bg-danger/5 p-5"><p className="font-medium">Не вдалося завантажити динаміку.</p><p className="mt-1 text-sm">{code === 'PROGRESS_NOT_FOUND' ? 'Для вибраного періоду або поточного плану даних немає.' : code === 'PROGRESS_INVALID_DATE_RANGE' ? 'Перевірте межі вибраного періоду.' : 'Спробуйте оновити сторінку.'}</p></div><a href={`/app/patients/${id}`} className="text-sm text-info underline">Повернутися до профілю</a></div>;
  }
}
