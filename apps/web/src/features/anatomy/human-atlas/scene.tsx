 ...
 useEffect(()=>{
  ...
  const materialFor=(system:string)=>{
   ...
   m.onBeforeCompile=shader=>{
    ...
    shader.vertexShader='attribute float partIndex; uniform sampler2D partState; uniform sampler2D selectionState; uniform float stateWidth; varying float partVisible; varying float partSelected; varying float partSeverity;\n'+shader.vertexShader;
    shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>','#include <begin_vertex>\nvec2 stateUv = vec2((partIndex + 0.5) / stateWidth, 0.5); vec4 state = texture2D(partState, stateUv); transformed += state.xyz; partVisible = state.w; vec4 clinicalState = texture2D(selectionState, stateUv); partSelected = clinicalState.r; partSeverity = clinicalState.g;');
    shader.fragmentShader='varying float partVisible; varying float partSelected; varying float partSeverity;\n'+shader.fragmentShader;
    shader.fragmentShader=shader.fragmentShader.replace('#include <clipping_planes_fragment>','#include <clipping_planes_fragment>\nif (partVisible < 0.5) discard;');
    shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>','#include <color_fragment>\nvec3 clinicalColor = partSeverity >= 7.0 ? vec3(0.86,0.15,0.15) : partSeverity >= 4.0 ? vec3(0.96,0.58,0.08) : vec3(0.25,0.77,0.35); diffuseColor.rgb = mix(diffuseColor.rgb, clinicalColor, min(partSeverity / 10.0, 0.72)); diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.42, 0.85, 0.78), partSelected * 0.75);');
  ...
  const up=(e:PointerEvent)=>{
   ...
   if(found<0&&amount>.45)found=findTarget(e.clientX-rect.left,e.clientY-rect.top,e.pointerType==='touch'?24:16);if(found>=0){hover.hidden=true;const hit=raycaster.intersectObject(pickers[found]!,false)[0];const part=atlas.parts[found];if(hit){const anchor=anchorForHit(pickers[found]!,hit);anchor.stableMeshKey=part.id;select.current({sourcePartId:part.id,conceptId:part.conceptId,anchor});} }
  };
  ...
  const animate=()=>{
   ...
   if(changed||moving||lastExtent<0){
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     const selected=selection.has(p.id),score=s.severity[p.id]??0;data.set([dx,dy,dz,(!s.hidden.includes(p.id)&&(s.isolate?selected:visible.has(p.system)||selected))?1:0],i*4);selectedData[i*4]=selected?255:0;selectedData[i*4+1]=Math.max(0,Math.min(10,score))*25.5;
